...
CREATE PROCEDURE PROC_INGRESOS(
   IN PI_INGRESOS  BIGINT
  ,IN PI_EGRESOS   BIGINT
  ,IN PV_INDENTIFICADOR  VARCHAR (1)
)
BEGIN
...
/* FUNCION 2 */
DECLARE PIL_INGRESOS2 INT;
DECLARE PIL_EGRESOS2 INT;
DECLARE PIL_CAPITAL INT;
/*8888888888888888888888 */
DECLARE PI_TOTAL INT;
START TRANSACTION;

/*INGRESOS*/
IF PV_INDENTIFICADOR = 1 THEN
SELECT SUM(INGRESOS) FROM CONTABILIDAD INTO PIL_INGRESOS;
SET PI_TOTAL := PIL_INGRESOS + PI_INGRESOS;
UPDATE contabilidad
SET 
   INGRESOS = PI_TOTAL -- bigint(20)
WHERE COD_CONTABILIDAD = 1; -- bigint(20)
SELECT SUM(CAPITAL) FROM CONTABILIDAD INTO PIL_CAPITAL;
SET PV_CONTROL := PIL_CAPITAL +  PI_INGRESOS; /* PI_INGRESOS ESTO ES LO QUE SE ENVIA POR LA API  */
UPDATE contabilidad
SET
  CAPITAL = PV_CONTROL -- bigint(20)

WHERE COD_CONTABILIDAD = 1; -- bigint(20)

/*EGRESOS*/
ELSEIF PV_INDENTIFICADOR = 2 THEN
SELECT SUM(EGRESOS) FROM CONTABILIDAD INTO PIL_EGRESOS;
SET PI_TOTAL := PIL_EGRESOS + PI_EGRESOS;
UPDATE contabilidad
SET 
  EGRESOS = PI_TOTAL -- bigint(20)
WHERE COD_CONTABILIDAD = 1; -- bigint(20)
/* ACTUALIZAR CAPITAL */
SELECT SUM(CAPITAL) FROM CONTABILIDAD INTO PIL_CAPITAL;
SET PV_CONTROL := PIL_CAPITAL -  PI_EGRESOS; /* PI_EGRESOS ESTO ES LO QUE SE ENVIA POR LA API  */
UPDATE contabilidad
SET
  CAPITAL = PV_CONTROL -- bigint(20)
WHERE COD_CONTABILIDAD = 1; -- bigint(20)
...
ELSEIF PV_INDENTIFICADOR = 4 THEN
SELECT * 
FROM CONTABILIDAD;
END IF;
COMMIT;
...
Select @COD_PERSONA := MAX(COD_PERSONA) FROM personas INTO PI_PERSONAS;

SELECT @COD_EMPRESA := MAX(COD_EMPRESA) from Fabricante INTO PI_PROVEEDORES;

SELECT @COD_VENTA := MAX(COD_VENTA) from VENTAS INTO PI_VENTAS ;

SELECT @COD_COMPRA := MAX(COD_COMPRA) from COMPRAS INTO PI_COMPRAS ;

SELECT @COD_producto := MAX(COD_producto) from producto INTO PI_PRODUCTOS;
...
WHERE COD_CONTEO = 1; -- ojo aqui por alguna razon la tabla de conteo comienza con el id 8
SELECT * FROM CONTEO;
...
CREATE PROCEDURE PROC_IMAGEN(
    PI_ID BIGINT
    ,PB_IMAGEN MEDIUMBLOB

)BEGIN
START TRANSACTION;
INSERT INTO twot.imagen(
